...
export const follow = async (event : RequestEvent, user_id : number) => {
    const resp = await event.fetch("http://0.0.0.0:3000/post/follow?token=" + event.cookies.get("token"), {
        headers: new Headers([['Content-Type', 'application/json']]),
        method: "POST",
...
export const unfollow = async (event : RequestEvent, user_id : number) => {
    const resp = await event.fetch("http://0.0.0.0:3000/post/unfollow?token=" + event.cookies.get("token"), {
        headers: new Headers([['Content-Type', 'application/json']]),
        method: "POST",
        body: JSON.stringify({follow : user_id}),
    });
